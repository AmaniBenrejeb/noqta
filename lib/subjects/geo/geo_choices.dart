...
class GeoChoices extends StatefulWidget {
  final String questionId;
  final Key? key;

  const GeoChoices({required this.questionId, this.key}) : super(key: key);

  @override
  _GeoChoicesState createState() => _GeoChoicesState();
}

class _GeoChoicesState extends State<GeoChoices> {
  ...
  Future<void> _initializeData() async {
    ...
    // Vérifier l'existence de la question dans la collection 'Geography'
    final geoSnapshot = await FirebaseFirestore.instance
        .collection('Geography')
        .doc(widget.questionId)
        .get();
    if (geoSnapshot.exists) {
      // Si la question existe dans la collection 'Geography'
      ...
      // Si la question n'existe pas dans la collection 'Geography', la chercher dans la collection 'Users'
      _questionStream = FirebaseFirestore.instance
          .collection('Users')
          .doc(userId)
            ...
            return Center(
              child: Container(
                ...
                decoration: BoxDecoration(
                  ...
                  border: Border.all(color: const Color(0xFF7D79F7)),
                ),
                child: Column(
  ...
  builder: (context, snapshot) {
    if (snapshot.connectionState == ConnectionState.waiting) {
      ...
      List<String> choices = snapshot.data!.docs.map((doc) => doc['answer'] as String).toList();
